...
function SidebarProvider({
	...
	const setOpen = useCallback(
		(nextOpen: boolean | ((prevOpen: boolean) => boolean)) => {
			...
			// biome-ignore lint/suspicious/noDocumentCookie: Sidebar state cookies
...
function SidebarRail({ className, ...props }: SidebarRailProps) {
	...
	return (
		<button
			...
			className={cn(
				'-translate-x-1/2 group-data-[side=left]:-right-4 absolute inset-y-0 z-20 hidden w-4 transition-all duration-75 ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] hover:after:bg-sidebar-border group-data-[side=right]:left-0 sm:flex',
...
function SidebarGroupLabel({
	...
	return (
		<Comp
			className={cn(
				'flex h-8 shrink-0 items-center rounded-md px-2 font-medium text-sidebar-foreground/70 text-xs outline-hidden ring-sidebar-ring transition-[margin,opacity] duration-100 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
